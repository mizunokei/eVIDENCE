...
for line in out_f:
	...
	if line_l[3].startswith("-") and len(re.split(r"\d+", line_l[-2])) != 3:
		continue
	if line_l[3].startswith("+") and len(re.split(r"\d+", line_l[-2])) != 2:
		continue
	...
	if name != pre_name and pre_name != "":
		...
		print(result_l[0]+"\t"+result_l[1]+"\t"+result_l[2]+"\t"+result_l[3]+"\t"+result_l[4]+"\t"+str(counter)+"\t"+str(counter/int(result_l[4])*100))
		counter = 0
		barcode_l = []
	counter += 1
	pre_name = name
