...
for line in out_f:
	...
	if line_l[0].startswith("#"):
		print(line_l[0],"\t",line_l[1],"\t",line_l[2],"\t",line_l[3],"\t",line_l[4],"\tvar_family_number","\tVAF","\t",line_l[5],"\t",line_l[6],"\t",line_l[7],"\t",line_l[8],"\t",line_l[9],"\t",line_l[10],"\t",line_l[11],"\t",line_l[12])
		continue
	...
	if name != pre_name and pre_name != "":
		...
		del_count = Counter(var_num_l)[1]+Counter(var_num_l)[2]
		for l in result_l:
			l_l = l.split()
			if int(l_l[7]) > 2:
